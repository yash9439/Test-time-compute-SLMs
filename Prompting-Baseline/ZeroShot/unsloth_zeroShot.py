...
def process_problem(problem_text, tokenizer, model):
    ...
    prompt = f"<｜User｜>{problem_text}<｜Assistant｜>Let's think step by step:\n"
...
df_aime24.to_csv("aime24_ZeroShot.csv", index=False)
...
# Save the updated dataset as a CSV file
df_math500.to_csv("math500_ZeroShot.csv", index=False)
...
df_gpqa.to_csv("gpqa_ZeroShot.csv", index=False)
